...
class OpenIDConnectTest extends Base
{
    public function testAuthorizationCodeSuccessWithIdToken()
    {
        ...
        $this->assertRegExp('{"access_token":"[^"]+","expires_in":[0-9]+,"scope":"openid","token_type":"Bearer","foo":"bar","id_token":"[^"]+"}', $response->getBody()->getContents());
    ...
    public function getValidIdToken()
    {
        ...
        $this->assertEquals(200, $response->getStatusCode());
        $this->assertEquals('no-cache', $response->getHeader('Pragma')[0]);
        $this->assertRegExp('{"access_token":"[^"]+","expires_in":[0-9]+,"scope":"[^"]+","token_type":"Bearer","foo":"bar","id_token":"[^"]+"}', $response->getBody()->getContents());
    ...
    public function testCodeTokenSuccessWithRefreshToken()
    {
        ...
        $this->getAuthorizationEndpoint()->setIsAuthorized(true);
        $this->getAuthorizationEndpoint()->authorize($request, $response);
        $this->assertRegExp('/^http:\/\/example.com\/test\?good=false#code=[^"]+&access_token=[^"]+&expires_in=[\d]+&scope=openid\+offline_access&token_type=Bearer&foo=bar&state=ABCDEF&session_state=[^"]+$/', $response->getHeader('Location')[0]);
        ...
        $this->assertEquals(200, $response->getStatusCode());
        $this->assertEquals('no-cache', $response->getHeader('Pragma')[0]);
        $this->assertRegExp('/^{"access_token":"[^"]+","expires_in":[\d]+,"scope":"openid offline_access","refresh_token":"[^"]+","token_type":"Bearer","foo":"bar","id_token":"[^"]+"}$/', $response->getBody()->getContents());
    ...
    public function testCodeTokenSuccessWithoutRefreshToken()
    {
        ...
        $this->getAuthorizationEndpoint()->setIsAuthorized(true);
        $this->getAuthorizationEndpoint()->authorize($request, $response);
        $this->assertRegExp('/^http:\/\/example.com\/test\?good=false#code=[^"]+&access_token=[^"]+&expires_in=[\d]+&scope=openid&token_type=Bearer&foo=bar&state=ABCDEF&session_state=[^"]+$/', $response->getHeader('Location')[0]);
        $values = parse_url($response->getHeader('Location')[0]);
        parse_str($values['fragment'], $params);
        ...
        $this->assertEquals(200, $response->getStatusCode());
        $this->assertEquals('no-cache', $response->getHeader('Pragma')[0]);
        $this->assertRegExp('/^{"access_token":"[^"]+","expires_in":[\d]+,"scope":"openid","token_type":"Bearer","foo":"bar","id_token":"[^"]+"}$/', $response->getBody()->getContents());
    ...
    public function testIdTokenTokenSuccess()
    {
        ...
        $this->getAuthorizationEndpoint()->setIsAuthorized(true);
        $this->getAuthorizationEndpoint()->authorize($request, $response);
        $this->assertRegExp('/^http:\/\/example.com\/test\?good=false#access_token=[^"]+&expires_in=[\d]+&scope=openid&token_type=Bearer&foo=bar&id_token=[^"]+&state=ABCDEF&session_state=[^"]+$/', $response->getHeader('Location')[0]);
        $values = parse_url($response->getHeader('Location')[0]);
        parse_str($values['fragment'], $params);
    ...
    public function testCodeIdTokenTokenSuccess()
    {
        ...
        $this->getAuthorizationEndpoint()->setIsAuthorized(true);
        $this->getAuthorizationEndpoint()->authorize($request, $response);
        $this->assertRegExp('/^http:\/\/example.com\/test\?good=false#code=[^"]+&access_token=[^"]+&expires_in=[\d]+&scope=openid&token_type=Bearer&foo=bar&id_token=[^"]+&state=ABCDEF&session_state=[^"]+$/', $response->getHeader('Location')[0]);
        $values = parse_url($response->getHeader('Location')[0]);
        parse_str($values['fragment'], $params);
    ...
    public function testPromptLoginWithFullyAuthenticatedUserAndConsentGiven()
    {
        ...
        $this->assertRegExp('/^http:\/\/example.com\/test\?good=false#access_token=[^"]+&expires_in=[\d]+&scope=openid\+email\+profile&token_type=Bearer&foo=bar&state=0123456789&session_state=[^"]+$/', $response->getHeader('Location')[0]);
    }
