...
class ImplicitGrantTypeTest extends Base
{
    ...
    public function testAccessTokenSuccess()
    {
        ...
        $response = new Response();
        $this->getAuthorizationEndpoint()->authorize($authorization, $response);
        $this->assertRegExp('/^http:\/\/example.com\/test\?good=false#access_token=[^"]+&token_type=Bearer&expires_in=[\d]+&scope=scope1\+scope2&foo=bar$/', $response->getHeader('Location')[0]);
    }
