...
abstract class MacToken implements TokenTypeInterface
{
     ...
     * @var int
     */
    private $timestampLifetime = 30;

    /**
     * @var string
     */
    private $macAlgorithm = 'hmac-sha-256';
     ...
     */
    public function setTimestampLifetime(int $timestampLifetime)
    {
     ...
     */
    public function setMacAlgorithm(string $macAlgorithm)
    {
    ...
    private function getParametersToCheck(): array
    {
        return [
            'id'    => function ($value, AccessToken $accessToken) {
                return hash_equals($accessToken->getId()->getValue(), $value);
            },
            'ts'    => function ($value, AccessToken $accessToken) {
                return time() < $this->getTimestampLifetime() + (int) $value;
            },
            'nonce' => function ($value, AccessToken $accessToken) {
                return true;
            },
        ];
    }
